...
if ~isfield(config_out, 'H_init') || isempty(config_out.H_init)  % not given any inital encoding matrices. Fill these in.
    ...
    switch algorithm
        case {'nmf', 'cnmf', 'cmf', 'ccmf', 'cmfwisa', 'ccmfwisa'}
            for i = 1 : num_sources
                config_out.H_init{i} = max(rand(num_basis_elems, n), eps);
            end
        case {'seminmf', 'convexnmf', 'convexhullnmf', 'convexhullcnmf', 'convexhullccmf'}
            ...
            for i = 1 : num_sources
                ...
                config_out.H_init{i} = zeros(num_basis_elems, n);
                for j = 1 : n
...
if ~isfield(config_out, 'W_init') || isempty(config_out.W_init)  % not given any inital basis matrices. Fill these in.
    ...
    switch algorithm
        case {'nmf', 'cmfwisa'}
            for i = 1 : num_sources
                config_out.W_init{i} = max(rand(m, num_basis_elems), eps);
                config_out.W_init{i} = config_out.W_init{i} * diag(1 ./ sqrt(sum(config_out.W_init{i}.^2, 1)));
            end
        case {'cnmf', 'ccmfwisa'}
            for i = 1 : num_sources
                config_out.W_init{i} = rand(m, num_basis_elems, context_len);
                for t = 1 : context_len
        ...
        case 'cmf'
            for i = 1 : num_sources
                config_out.W_init{i} = (2 * rand(m, num_basis_elems) - 1) + 1j*(2 * rand(m, num_basis_elems) - 1);
                config_out.W_init{i} = config_out.W_init{i} * diag(1 ./ sqrt(real(diag(config_out.W_init{i}' * config_out.W_init{i}))));
            end
        case 'ccmf'
            for i = 1 : num_sources
                config_out.W_init{i} = (2 * rand(m, num_basis_elems, context_len) - 1) + 1j*(2 * rand(m, num_basis_elems, context_len) - 1);
                for t = 1 : context_len
        ...
        case 'seminmf'
            for i = 1 : num_sources
                config_out.W_init{i} = 2*rand(m, num_basis_elems) - 1;
            end
